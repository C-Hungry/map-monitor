...
import zkBMapPlayback from '../../../components/zkBMapPlaybackH5.vue';
...
export default {
  ...
  data() {
    return {
      ...
      isPlay: false,
      simId: "",
      datePickerValue: [this.$utils.formateDate(new Date(new Date().getTime() - 1000 * 60 * 60), "yyyy-MM-dd HH:mm"), this.$utils.formateDate(new Date(), "yyyy-MM-dd HH:mm")],
      startTime: this.$utils.formateDate(new Date(new Date().getTime() - 1000 * 60 * 60), "yyyy-MM-dd HH:mm"),
      endTime: this.$utils.formateDate(new Date(), "yyyy-MM-dd HH:mm"),
      speed: "", // 速度
      allPoints: [], // 所有坐标点集合
      carNumberList: [],
      startPoint: { // 起点
  ...
  methods: {
    ...
    getHistoryRoute() {
      ...
      this.$bizService.get("/GPSService/GetHisGps",{
        ...
        this.$refs['zkMapPlaybackH5'].initMapData(this.allPoints);
        ...
        // this.drawStartAndEnd(this.allPoints[0], this.allPoints[this.allPoints.length - 1]);
        // this.centerAndZoom(this.allPoints);
        // this.drawRoute(this.allPoints);
