import zkBMapMonitorH5 from '../../../components/zkBMapMonitorH5.vue';
  ...
  methods: {
    ...
    onSelectChange(simId) {
      if (!simId) {
        ...
        this.$refs['zkMapMonitorH5'].onInfoWindowClose();
        for(let i=0;i<this.allPoints.length;i++) {
          if(this.allPoints[i].SimId == simId) {
            this.$refs['zkMapMonitorH5'].openInfoWindow(this.allPoints[i], i);
            break;
          } 
        }
      }
    ...
    initWebSocket() {
      ...
      */
      if (!this.isFirst) {
        this.$refs['zkMapMonitorH5'].initMap(this.allPoints, this.center, this.zoom);
        return;
      }
      ...
      try {
        ...
        Promise.all(promiseArrey)
          .then(results => {
            ...
            if (!this.isFirst) {
              this.$refs['zkMapMonitorH5'].initMap(this.allPoints, null, null, true);
            } else {
              this.$refs['zkMapMonitorH5'].initMap(this.allPoints);
            }
    ...
    statisticByStatus() {
      ...
      this.statistic.lost = this.allCarNumberList.length - this.statistic.running - this.statistic.parking- this.statistic.offline;
      this.isShowTable && this.getSimIdListByStatus(this.tagStatus);
    },
    // 获取车辆及simId
    getAllCarList() {
      ...
      .then(res => {
        ...
        res.data.forEach(item => {
          if (allCurrentSimIds.indexOf(item.gpsId) == -1) {
