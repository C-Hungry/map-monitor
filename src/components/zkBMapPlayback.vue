  ...
  methods: {
    ...
    drawHistoryRoute(points) {
      ...
      points.forEach((item, index) => {
        ...
        polyline.addEventListener('mouseout', (result) => {
          this.hoverMarker && this.bMap.removeOverlay(this.hoverMarker);
        })

        polyline.addEventListener('mouseover', (result) => {
          this.bMap.removeOverlay(this.hoverMarker);
          // 关闭上一个信息窗体
          this.bMap.closeInfoWindow(); 
          // 添加marker点
          var point = startPoint;
          this.hoverMarker = new BMap.Marker(point, {
            truckNo: point.CarNum,
            icon: new BMap.Icon(this.iconPoint, new BMap.Size(16, 16))
          })
          this.bMap.addOverlay(this.hoverMarker);

          // 添加label
          var label = new BMap.Label(item.GpsTime,{offset:new BMap.Size(26, 0)});
          label.setStyle({
            fontSize: '12px',
            borderColor: '#ccc',
            padding: '1px 3px'
          });
          this.hoverMarker.setLabel(label);
          

          this.hoverMarker.addEventListener("click", (event) => {
            // zIndex设置为最高
            this.hoverMarker.setTop(true);
            // 关闭上一个信息窗体
            this.bMap.closeInfoWindow(); 
            // 地址解析
            ...
            try {
              geocoder.getLocation(new BMap.Point(item.lng, item.lat), (result) => {
                ...
                let infoWindow = new BMap.InfoWindow(iwContent);
